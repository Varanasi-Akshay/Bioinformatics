...
ggplot(iris,aes(x=Petal.Length,y=Sepal.Length,color=Species))+geom_boxplot()
```
...
```{r}
# R code goes here.
ggplot(sitka,aes(x=Time,y=size))+geom_point()+geom_line(aes(group=tree))+facet_wrap(~treat)
```
...
ggplot(sitka,aes(x=Time,y=size,color=tree))+geom_point()+geom_line(aes(group=tree))+facet_wrap(~treat)
```
...
ggplot(sitka,aes(x=Time,y=size,color=size))+geom_point()+geom_line(aes(group=tree))+facet_wrap(~treat)
```
...
# one way to factor
#sitka$size <- factor(sitka$size)
ggplot(sitka,aes(x=Time,y=size,group=tree,color=factor(tree)))+geom_point()+geom_line()+facet_wrap(~treat)#scale_color_hue(h = c(0, 360)+15)#+scale_color_discrete()+scale_colour_gradientn(colours=rainbow(7))
...
```{r}
ggplot(iris, aes(x=Sepal.Length, y=Petal.Length, color=Species)) + geom_point() + geom_smooth(aes(group=Species), method=lm, color='black', se=F)    # se for confidence interval # method = lm (linear model)
