...
Variables present in this dataset are Year, Month and Number. So instead of columns having name of the month as their column name, there should be a column for months under which months should be put. Similarly for years, they are each row instead of them being under one column.
...
I pick the ToothGrowth dataset
```{r}
#data()
head(ToothGrowth)
```

The dataset is Tidy because it satisfies all the three conditions of a Tidy data. Each variable length, suppliment and dose has its column and each observation here in this case is the patient has his own row.  
...
The flight 1510 has longest mean delay of 283.
