...
```{r}
# R code goes here
locs <- flavors_of_cacao %>% count(location) %>% filter(n>20)

df <- flavors_of_cacao %>%
  filter(location %in% locs$location) %>% group_by(location,company) %>%
  summarise(num_reviews = n(), min_rating = min(rating), max_rating = max(rating)) %>%
